...
void main(uint3 groupId : SV_GroupID, uint3 threadId : SV_GroupThreadID) {
	...
	if (oid < len) {
		p0 = pixels.Load(iid);
		storeColor(oid, w, (p0 >> 16) & 0xFF, (p0 >> 8) & 0xFF, p0 & 0xFF);
	}
	if (oid + 1 < len) {
		p1 = pixels.Load(iid + 4);
		storeColor(oid + 1, w, (p1 >> 8) & 0xFF, p1 & 0xFF, p0 >> 24);
	}
	if (oid + 2 < len) {
		p2 = pixels.Load(iid + 8);
		storeColor(oid + 2, w, p2 & 0xFF, p1 >> 24, (p1 >> 16) & 0xFF);
	}
	if (oid + 3 < len) {
		storeColor(oid + 3, w, p2 >> 24, (p2 >> 16) & 0xFF, (p2 >> 8) & 0xFF);
	}
}
