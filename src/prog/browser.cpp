...
void Browser::LoadProgress::pushImage(BrowserResultPicture* rp, Cstring&& name, SDL_Surface* img, const PicLim& picLim, uintptr_t imgSize) {
	rp->cnt = ++c;
	m += imgSize;
	pushEvent(SDL_USEREVENT_THREAD_READER, ThreadEvent::progress, new BrowserPictureProgress(img, rp->pics.emplace_front(std::move(name), nullptr).second, std::format("Loading {}{}", numStr(picLim, c, m), suffix)));
}
