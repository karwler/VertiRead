...
bool FileOpsLocal::pollWatch(vector<FileChange>& files) {
	...
	if (dirc != INVALID_HANDLE_VALUE)
		while (WaitForSingleObject(overlapped.hEvent, 0) == WAIT_OBJECT_0) {
			...
			for (auto event = reinterpret_cast<FILE_NOTIFY_INFORMATION*>(reinterpret_cast<void*>(ebuf));; event = reinterpret_cast<FILE_NOTIFY_INFORMATION*>(reinterpret_cast<void*>(reinterpret_cast<byte*>(event) + event->NextEntryOffset))) {
