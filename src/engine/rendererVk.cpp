...
Texture* RendererVk::texFromText(const Pixmap& pm) noexcept {
	if (pm.res.x) {
		...
		try {
			prepareTexture(*tex, pm.pix.get(), pm.res.x, 1, VK_FORMAT_R8_UNORM, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT, { .r = VK_COMPONENT_SWIZZLE_ONE, .g = VK_COMPONENT_SWIZZLE_ONE, .b = VK_COMPONENT_SWIZZLE_ONE, .a = VK_COMPONENT_SWIZZLE_R });
...
bool RendererVk::texFromText(Texture* tex, const Pixmap& pm) noexcept {
	if (pm.res.x) {
		...
		try {
			prepareTexture(ntex, pm.pix.get(), pm.res.x, 1, VK_FORMAT_R8_UNORM, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT, { .r = VK_COMPONENT_SWIZZLE_ONE, .g = VK_COMPONENT_SWIZZLE_ONE, .b = VK_COMPONENT_SWIZZLE_ONE, .a = VK_COMPONENT_SWIZZLE_R });
