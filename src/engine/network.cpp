...
FtpReply FtpReceiver::getReply(NetConnection& conn) {
	...
	ushort rcode;
	std::from_chars_result crs = cmdEnd - cmdPos == 3 ? std::from_chars(cmdPos, cmdEnd, rcode, 10) : std::from_chars_result{ .ec = std::errc::invalid_argument };
	FtpReply reply = {
		.cmd = string(cmdPos, cmdEnd),
		.args = cmdEnd != line.end() ? string(cmdEnd + 1, line.end()) : string(),
		.code = crs.ptr == cmdEnd && crs.ec == std::errc() ? rcode : ushort(0),
...
Data FtpReceiver::getData(NetConnection& conn, size_t initRsv) {
	...
	uptr<byte[]> data = std::make_unique_for_overwrite<byte[]>(drsv);
