...
#include "shaders/dxIdxCs.dbg.h"
	...
	if (HRESULT rs = dev->CreateComputeShader(srcRgb, sizeof(srcRgb), nullptr, &compConv[eint(FormatConv::rgb24)]); FAILED(rs))
		SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to create rgb compute shader: %s", hresultToStr(rs).data());
	if (HRESULT rs = dev->CreateComputeShader(srcBgr, sizeof(srcBgr), nullptr, &compConv[eint(FormatConv::bgr24)]); FAILED(rs))
		SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to create bgr compute shader: %s", hresultToStr(rs).data());
	if (HRESULT rs = dev->CreateComputeShader(srcRed, sizeof(srcRed), nullptr, &compConv[eint(FormatConv::red)]); FAILED(rs))
		SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to create red compute shader: %s", hresultToStr(rs).data());
	if (HRESULT rs = dev->CreateComputeShader(srcIdx, sizeof(srcIdx), nullptr, &compConv[eint(FormatConv::index8)]); FAILED(rs))
		SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to create index compute shader: %s", hresultToStr(rs).data());
	...
	if (SurfaceInfo si = pickPixFormat(limitSize(img, D3D11_REQ_TEXTURE2D_U_OR_V_DIMENSION)); si.img) {
		try {
			...
			ID3D11ShaderResourceView* view = si.fmt
				...
				: createTextureIndirect(si.img->pixels, res, surfaceBytesPpx(si.img), si.img->pitch, surfacePalette(si.img), si.fcid);
	...
	if (pm.res.x) {
		try {
			...
			return new TextureDx(res, compConv[eint(FormatConv::red)]
				? createTextureIndirect(pm.pix.get(), res, 1, pm.res.x, nullptr, FormatConv::red)
...
ID3D11ShaderResourceView* RendererDx11::createTextureDirect(const void* pix, uvec2 res, uint pitch, DXGI_FORMAT format) {
...
ID3D11ShaderResourceView* RendererDx11::createTextureIndirect(const void* pix, uvec2 res, uint8 bpp, uint pitch, const SDL_Palette* palette, FormatConv fcid) {
	ID3D11Texture2D* texture = nullptr;
	ID3D11ShaderResourceView* view = nullptr;
