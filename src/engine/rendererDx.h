...
private:
	...
	ID3D11ShaderResourceView* createTextureDirect(const void* pix, uvec2 res, uint pitch, DXGI_FORMAT format);
	ID3D11ShaderResourceView* createTextureIndirect(const void* pix, uvec2 res, uint8 bpp, uint pitch, const SDL_Palette* palette, FormatConv fcid);
