...
public:
	Data() = default;
	Data(size_t siz) : ptr(std::make_unique_for_overwrite<byte[]>(siz)), len(siz) {}
	Data(uptr<byte[]>&& p, size_t l) : ptr(std::move(p)), len(l) {}

	byte* data() { return ptr.get(); }
	const byte* data() const { return ptr.get(); }
...
T toNum(string_view str, A... args) {
	...
	return std::from_chars(str.data() + i, str.end(), val, args...).ec == std::errc() ? val : T(0);
}
