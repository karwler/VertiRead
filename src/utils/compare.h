...
public:
	...
	template <class C, class T, class A> bool operator()(const std::basic_string<C, T, A>& a, const std::basic_string<C, T, A>& b);
...
inline bool StrNatCmp::operator()(const fs::path& a, const fs::path& b) {
	return cmp(a.c_str(), b.c_str()) < 0;
}

template <class C, class T, class A>
bool StrNatCmp::operator()(const std::basic_string<C, T, A>& a, const std::basic_string<C, T, A>& b) {
	return cmp(a.c_str(), b.c_str()) < 0;
}

template <class C>
bool StrNatCmp::operator()(const C* a, const C* b) {
	return cmp(a, b) < 0;
}
